...
from flask import redirect, Flask, jsonify, request, session
from flask_cors import CORS

def get_duval_algolia_client():
    """
    Small helper to build the Algolia client for Duval.
    Reuses the existing DUVAL_ALG_APP_ID and DUVAL_ALG_API_KEY constants.
    """
    from algoliasearch import algoliasearch
    return algoliasearch.Client(DUVAL_ALG_APP_ID, DUVAL_ALG_API_KEY)


# DISABLED_DUPLICATE_DEF def get_duval_algolia_client():
# DISABLED_DUPLICATE_DEF     """
# DISABLED_DUPLICATE_DEF     Small helper to build the Algolia client for Duval.
# DISABLED_DUPLICATE_DEF     Reuses the existing DUVAL_ALG_APP_ID and DUVAL_ALG_API_KEY constants.
# DISABLED_DUPLICATE_DEF     """
# DISABLED_DUPLICATE_DEF     from algoliasearch import algoliasearch
# DISABLED_DUPLICATE_DEF     return algoliasearch.Client(DUVAL_ALG_APP_ID, DUVAL_ALG_API_KEY)
# DISABLED_DUPLICATE_DEF 
# DISABLED_DUPLICATE_DEF 
# DISABLED_DUPLICATE_DEF # ------------------------------------------------------------------------------
# DISABLED_DUPLICATE_DEF # Config
# DISABLED_DUPLICATE_DEF # ------------------------------------------------------------------------------
# DISABLED_DUPLICATE_DEF 
# DISABLED_DUPLICATE_DEF # Duval Algolia public search config (this is what the browser uses)
# DISABLED_DUPLICATE_DEF DUVAL_ALG_APP_ID = "0LWZO52LS2"
# DISABLED_DUPLICATE_DEF DUVAL_ALG_API_KEY = "c0745578b56854a1b90ed57b63fbf0ba"
# DISABLED_DUPLICATE_DEF DUVAL_ALG_INDEX = "fl-duval.property_tax"
# DISABLED_DUPLICATE_DEF DUVAL_ALG_ENDPOINT = (
# DISABLED_DUPLICATE_DEF     f"https://{DUVAL_ALG_APP_ID}-dsn.algolia.net/1/indexes/*/queries"
# DISABLED_DUPLICATE_DEF )
# DISABLED_DUPLICATE_DEF 
# DISABLED_DUPLICATE_DEF # CSV & cache
# DISABLED_DUPLICATE_DEF CSV_PATH = os.getenv("CSV_PATH", "leads.csv")
# DISABLED_DUPLICATE_DEF CACHE_DAYS = int(os.getenv("CACHE_DAYS", "30"))
# DISABLED_DUPLICATE_DEF 
# DISABLED_DUPLICATE_DEF # Base for Duval public site (bills page)
# DISABLED_DUPLICATE_DEF DUVAL_BASE_URL = "https://county-taxes.net"
# DISABLED_DUPLICATE_DEF 
# DISABLED_DUPLICATE_DEF # ------------------------------------------------------------------------------
# DISABLED_DUPLICATE_DEF # Flask app
# DISABLED_DUPLICATE_DEF # ------------------------------------------------------------------------------
# DISABLED_DUPLICATE_DEF 
# DISABLED_DUPLICATE_DEF app = Flask(__name__)
# DISABLED_DUPLICATE_DEF app.secret_key = os.getenv('SECRET_KEY', 'dev-secret-change-me')
# DISABLED_DUPLICATE_DEF CORS(app)
# DISABLED_DUPLICATE_DEF 
# DISABLED_DUPLICATE_DEF # ------------------------------------------------------------------------------
# DISABLED_DUPLICATE_DEF # CSV helpers
# DISABLED_DUPLICATE_DEF # ------------------------------------------------------------------------------
# DISABLED_DUPLICATE_DEF 
# DISABLED_DUPLICATE_DEF CSV_FIELDS = [
# DISABLED_DUPLICATE_DEF     "parcel",
# DISABLED_DUPLICATE_DEF     "owner_name",
# DISABLED_DUPLICATE_DEF     "display_name",
# DISABLED_DUPLICATE_DEF     "address",
# DISABLED_DUPLICATE_DEF     "city",
# DISABLED_DUPLICATE_DEF     "state",
# DISABLED_DUPLICATE_DEF     "zip",
# DISABLED_DUPLICATE_DEF     "public_url",
# DISABLED_DUPLICATE_DEF     "total_due",
# DISABLED_DUPLICATE_DEF     "delinquent_due",
# DISABLED_DUPLICATE_DEF     "last_year_due",
# DISABLED_DUPLICATE_DEF     "total_due_numeric",
# DISABLED_DUPLICATE_DEF     "is_distressed",
# DISABLED_DUPLICATE_DEF     "source",
# DISABLED_DUPLICATE_DEF     "created_at",
# DISABLED_DUPLICATE_DEF ]
# DISABLED_DUPLICATE_DEF 
# DISABLED_DUPLICATE_DEF 
def csv_exists() -> bool:
...
def fetch_duval_bill_amounts(public_url: str, debug: bool = False):
    ...
    try several strategies to get the **Total Amount Due**:
      ...
      1) JSON endpoint (bills?format=json) Ã¢â‚¬â€œ usually empty, but harmless to try.
...
@app.route("/api/search_parcel")
def search_parcel():
    parcel = request.args.get("parcel", "").strip()
    if not parcel:
        return jsonify({"status": "error", "message": "parcel param missing"}), 400
    try:
        client = get_duval_algolia_client()
        index = client.init_index(DUVAL_ALG_INDEX)
        res = index.search("", { "filters": f"parcel:{parcel}" })
        return jsonify({
            "status": "ok",
            "count": res.get("nbHits", 0),
            "rows": res.get("hits", [])
        })
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500

from flask import jsonify, request, send_from_directory, redirect, redirect
import csv
from pathlib import Path
...
def _load_delinquent_rows():
    if not DELINQ_CSV.exists():
        return []
    with DELINQ_CSV.open(newline="", encoding="utf-8") as f:
...
@app.get("/admin.html")
def admin_html_page():
    return (send_from_directory(BASE_DIR, "admin.html") if _require_admin() else redirect("/login"))
...
def api_health():
    return jsonify({
        "ok": True,
        "delinquent_csv_exists": DELINQ_CSV.exists(),
        "delinquent_csv_path": str(DELINQ_CSV),
    })
...
def api_delinquent():
    # Query params:
    # q= (search address/owner/parcel)
    # zip=
    # min_due=
    # still= (true/false) optional
    # page=1, page_size=50
...
def api_rebuild_delinquent():
    # Safe: just run the python script that rebuilds the CSV
    import subprocess, sys
    script = BASE_DIR / "duval_build_big_delinquent.py"
    if not script.exists():
        return jsonify({"ok": False, "error": "duval_build_big_delinquent.py not found"}), 500

    p = subprocess.run([sys.executable, str(script)], capture_output=True, text=True, cwd=str(BASE_DIR))
    return jsonify({
        "ok": (p.returncode == 0),
...
# DISABLED_DUPLICATE_API_LOGIN @app.post("/api/login")
# DISABLED_DUPLICATE_API_LOGIN def api_login():
# DISABLED_DUPLICATE_API_LOGIN     # accepts JSON or form
# DISABLED_DUPLICATE_API_LOGIN     data = {}
# DISABLED_DUPLICATE_API_LOGIN     try:
# DISABLED_DUPLICATE_API_LOGIN         data = request.get_json(silent=True) or {}
# DISABLED_DUPLICATE_API_LOGIN     except Exception:
# DISABLED_DUPLICATE_API_LOGIN         data = {}
# DISABLED_DUPLICATE_API_LOGIN     pw = (data.get("password") if isinstance(data, dict) else None) or request.form.get("password") or request.values.get("password") or ""
# DISABLED_DUPLICATE_API_LOGIN     pw = str(pw).strip()
# DISABLED_DUPLICATE_API_LOGIN     if pw == str(ADMIN_PASSWORD):
# DISABLED_DUPLICATE_API_LOGIN         session["is_admin"] = True
# DISABLED_DUPLICATE_API_LOGIN         return jsonify({"ok": True})
# DISABLED_DUPLICATE_API_LOGIN     return jsonify({"ok": False, "error": "Invalid password"}), 401
# DISABLED_DUPLICATE_API_LOGIN 
# DISABLED_DUPLICATE_ENDPOINT @app.post("/api/logout")
# DISABLED_DUPLICATE_ENDPOINT def api_logout():
# DISABLED_DUPLICATE_ENDPOINT     session.clear()
# DISABLED_DUPLICATE_ENDPOINT     return jsonify({"ok": True})
# DISABLED_DUPLICATE_ENDPOINT 
@app.get("/api/me")
def api_me():
