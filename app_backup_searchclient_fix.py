...
def fetch_duval_bill_amounts(public_url: str, debug: bool = False):
    ...
    try several strategies to get the **Total Amount Due**:
      ...
      1) JSON endpoint (bills?format=json) â€“ usually empty, but harmless to try.
...
def search_parcel():
    ...
    digits = "".join(c for c in parcel_raw if c.isdigit())
    if len(digits) < 6:
        return jsonify({"status": "error", "message": "invalid parcel"}), 400

    prefix = digits[:6]
    suffix = digits[6:10].ljust(4, "0")
    ...
    try:
        client = SearchClient.create(DUVAL_ALG_APP_ID, DUVAL_ALG_API_KEY)
        index = client.init_index(DUVAL_ALG_INDEX)

        for p in candidates:
            res = index.search("", {"filters": f"parcel:{p}"})
            hits = res.get("hits", [])

            for h in hits:
                alg_parcel = "".join(c for c in h.get("parcel", "") if c.isdigit())
                if alg_parcel == digits:
                    return jsonify({
                        "status": "ok",
                        "matched_parcel": p,
                        "count": 1,
